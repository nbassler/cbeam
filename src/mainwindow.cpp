    ...
    m_model(new Model(this))
{
    ...
    for (QDoubleSpinBox *box : { ui->doubleSpinBox_goPos,
                                 ui->doubleSpinBox_llim,
    ...
    connect(ui->horizontalSlider, &QSlider::valueChanged,
            m_model, &Model::setTargetSteps);
    connect(ui->doubleSpinBox_goPos, &QDoubleSpinBox::valueChanged,
            m_model, &Model::setTargetMm);
    connect(ui->doubleSpinBox_llim, &QDoubleSpinBox::valueChanged,
            m_model, &Model::setLlimMm);
    connect(ui->doubleSpinBox_ulim, &QDoubleSpinBox::valueChanged,
            m_model, &Model::setUlimMm);

    setUpJogColumns();

    ui->checkBox_lockLimits->setChecked(CB_LOCK_LIMITS_AT_STARTUP);
    connect(ui->checkBox_lockLimits, &QCheckBox::toggled,
            this, &MainWindow::updateControlStates);
    ...
    auto *calibration = new QLabel(
        tr("%1 steps/cm | %2 mm/step")
        .arg(CB_STEPS_PER_CM, 0, 'g', 6)
        .arg(CB_MM_PER_STEP, 0, 'f', CB_MM_DIGITS + 3), this);
...
{
    m_jog = { {
                  { ui->pushButton_plusA, ui->pushButton_minusA,
                    ui->spinBox_jogA, ui->label_jogMmA },
                  { ui->pushButton_plusB, ui->pushButton_minusB,
                    ui->spinBox_jogB, ui->label_jogMmB },
                  { ui->pushButton_plusC, ui->pushButton_minusC,
                    ui->spinBox_jogC, ui->label_jogMmC },
              } };
    ...
    for (int i = 0; i < CB_JOG_COLUMNS; i++)
    {
        ...
        connect(col.plus, &QPushButton::clicked, this, [this, &col] {
            m_model->jog(+col.steps->value());
        });
        connect(col.minus, &QPushButton::clicked, this, [this, &col] {
            m_model->jog(-col.steps->value());
        });
        connect(col.steps, &QSpinBox::valueChanged, this, [this, &col] {
...
void MainWindow::relabelJogColumn(const JogColumn &col)
{
    ...
    const QString hint = tr("Jog %1 steps (%2 mm)")
                         .arg(steps)
                         .arg(mm, 0, 'f', CB_MM_DIGITS);
...
{
    ui->label_currentPos->setText(QString("%1 mm").arg(mm, 0, 'f',
                                                       CB_MM_DIGITS));
...
void MainWindow::showLimits(int loSteps, int hiSteps, double loMm, double hiMm)
{
    {
        const QSignalBlocker block(ui->horizontalSlider);
...
void MainWindow::updateControlStates()
{
    ...
    statusBar()->showMessage(QString("%1 - %2")
                             .arg(m_moving ? tr("Moving...") : tr("Idle"),
